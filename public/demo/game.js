...
(() => {
  ...
  const chapters=[
    ...
    {title:'Diese eine verrückte Nacht.',subtitle:'Manche Geschichten haben Kratzer.',short:'Die chaotische Nacht',tag:'EIN SPÄTERER DISCOBESUCH',intro:'Nur Luana und ich, mitten in einem vollen Club. Um uns herum lauter fremde Leute. Dann zerbrechen Gläser, und alles geht plötzlich viel zu schnell.'},
  ...
  function startChapter(index){$('transition').classList.add('hidden');canvas.focus({preventScroll:true});if(index===0){setPhase('club');s.player.x=160;objective('Geh zu Maria. Sie ist nicht allein.');showDialog([{who:'Ich',text:'Eigentlich sollte es nur ein Abend mit Pere werden.'},{who:'Pere',text:'Da drüben ist doch Maria. Komm, wir sagen Hallo.'}],()=>setAction('Zu Maria gehen',()=>walkTo(750,580,meetMaria)));}if(index===1){setPhase('drive-intro');objective('Finde mit Luana einen ruhigen Parkplatz.');showDialog([{who:'Ich',text:'Am nächsten Tag. Du auf dem Beifahrersitz meines Fiat Bravo. Ein ziemlich gutes erstes Date, würde ich sagen.'},{who:'Luana',text:'Fahren wir noch ein Stück? Irgendwohin, wo es ruhig ist.'},{who:'Ich',text:'Lenke mit ← und →. Der Bravo rollt von allein. Weiche den anderen Autos aus! ↑ gibt Gas, ↓ bremst. Bei einem Zusammenstoß beginnt nur die Fahrt neu.'}],beginDrive);}if(index===2){setPhase('club-calm');objective('Nur wir zwei, mitten im vollen Club.');s.player.x=650;showDialog([{who:'Ich',text:'Diesmal sind nur wir zwei zusammen hier. Um uns herum ein voller Club, fremde Gesichter und laute Musik.'},{who:'Luana',text:'Komm, wir tanzen noch ein bisschen.'}],()=>setAction('Weiterfeiern',beginClubIncident));}if(index===3){setPhase('home');s.player.x=175;s.catX=660;s.catY=587;s.homeStep=0;objective('Lerne euer kleines Fellknäuel kennen.');showDialog([{who:'Ich',text:'Und dann kam jemand in unser Leben, der noch kleiner war als unsere Pläne.'},{who:'Luana',text:'Schau ihn dir an. Dieses Gesicht! Willkommen zu Hause, Pablo.'}],()=>setAction('Pablo begrüßen',()=>walkTo(s.catX-70,582,petKitten)));}if(index===4){setPhase('present');s.player.x=180;s.catX=480;s.catY=590;objective('Folge Pablo zum Laptop.');showDialog([{who:'Ich',text:'Die Nächte wurden Erinnerungen. Aus dem kleinen Fellknäuel wurde ein stattlicher Kater. Und aus uns wurde ein Zuhause.'},{who:'Ich',text:'Heute habe ich dir diesen QR-Code gegeben. Jetzt sitzen wir beide vor dem Laptop – genau wie gleich im Spiel.'}],()=>{setAction('Pablo folgen',()=>{s.catTarget={x:1160,y:495};walkTo(980,480,openLaptop);});});}}
  ...
  function renderDialog(){const line=s.dialog[s.dialogIndex];$('speaker').textContent=line.who;$('dialogue-text').textContent=line.text;$('next-dialogue').innerHTML='Weiter <span>↵</span>';const pc=$('portrait').getContext('2d');pc.clearRect(0,0,160,160);pc.fillStyle=line.who==='Luana'?'#c5d5c7':'#e2d4bb';pc.fillRect(0,0,160,160);const map={'Ich':'me','Luana':'luana','Pere':'pere','Maria':'maria','Pablo':'kitten'};if(spriteReady){const key=map[line.who]||'me';const head={me:[229,4,120,143],luana:[683,8,146,160],pere:[1190,0,141,166],maria:[205,610,172,156],kitten:[641,738,212,184]}[key];atlasDraw(pc,key,head,0,0,160,160);}}
  ...
  function meetMaria(){showDialog([{who:'Maria',text:'Hey! Schön, euch zu sehen. Das ist meine Freundin Luana.'},{who:'Luana',text:'Hi! Freut mich.'},{who:'Ich',text:'Ich weiß nicht mehr jedes Wort von diesem Abend. Aber ich weiß noch, dass ich unbedingt mit dir tanzen wollte.'},{who:'Ich',text:'Luana, hast du Lust zu tanzen?'},{who:'Luana',text:'Na los. Zeig, was du kannst.'}],()=>{objective('Geht zusammen auf die Tanzfläche.');setAction('Auf die Tanzfläche',()=>walkTo(555,575,()=>showDialog([{who:'Ich',text:'Tippe auf den leuchtenden Pfeil oder bewege den Joystick in diese Richtung. 12 richtige Schritte schaffen den Tanz. Ein falscher Druck? Du kannst sofort korrigieren. Nach drei gescheiterten Runden darfst du überspringen.'}],beginDance)));});}
  ...
  function finishDance(){s.dance.done=true;$('rhythm-controls').classList.add('hidden');audio.effect('win');showDialog([{who:'Pere',text:'Okay, ihr habt jetzt wirklich die ganze Tanzfläche übernommen.'},{who:'Maria',text:'Die beiden hören heute nicht mehr auf, oder?'},{who:'Ich',text:'Wir haben Rambazamba getanzt. Für einen Moment war der ganze Raum nur für uns.'},{who:'Luana',text:'Das machen wir wieder.'}],()=>showTransition('ERINNERUNG 01 GESAMMELT','Ein Tanz. Und plötzlich wir.','So fing es an. Nicht mit einem perfekten Satz. Sondern damit, dass wir gemeinsam die ganze Tanzfläche eingenommen haben.',()=>enterChapter(1),'Zum ersten Date →'));}
  ...
  function finishDrive(){s.drive.parked=true;setPhase('parking');objective('Ein ruhiger Ort. Nur ihr zwei.');showDialog([{who:'Luana',text:'Hier ist es schön. Bleiben wir noch ein bisschen?'},{who:'Ich',text:'Der Motor aus. Die Welt ein bisschen leiser. Und du neben mir.'}],()=>setAction('Den Moment genießen',()=>{setPhase('private-moment');objective('Manche Erinnerungen gehören nur uns.');audio.effect('heart');}));}
  function finishPrivate(){showDialog([{who:'Ich',text:'Sagen wir so: Der Fiat Bravo hat diesen Parkplatz nicht vergessen.'},{who:'Ich',text:'Und was wir beide nicht vergessen haben, braucht hier keine große Erklärung.'}],()=>showTransition('ERINNERUNG 02 GESAMMELT','Ein Parkplatz irgendwo.','Es war kein besonderer Ort. Er wurde einer, weil du dort warst.',()=>enterChapter(2),'Eine andere Nacht →'));}
  ...
  function finishClubIncident(){beginChase();showDialog([{who:'Ich',text:'Draußen geht es weiter. Springe mit Leertaste oder ↑ über jedes Hindernis. Wenn du stolperst, beginnt nur die Flucht neu – nicht der Abend im Club.'}],()=>canvas.focus({preventScroll:true}));}
  ...
  function failChallenge(kind){
    ...
    const copy={
      ...
      drive:['Bravo. Noch ein Versuch.','Such die freie Lücke und ziehe den Joystick nach unten zum Bremsen.'],
  ...
  function finishPabloGame(){
    ...
    audio.effect('win');showDialog([{who:'Luana',text:'Zehn von zehn. Ich glaube, Pablo hat das Spiel gewonnen.'},{who:'Ich',text:'Ein roter Punkt, ein paar Leckerlis und ein ganz normaler Abend. Manchmal sind genau das die Erinnerungen, die bleiben.'}],()=>{setPhase('growing');objective('Kleine Pfoten. Große gemeinsame Erinnerungen.');});
  }
  function finishGrowing(){setPhase('home-grown');s.catX=660;s.catY=578;objective('Kuschle mit dem großen Pablo.');setAction('Zu Pablo aufs Sofa',()=>walkTo(630,578,()=>showDialog([{who:'Luana',text:'Weißt du noch, wie klein er war?'},{who:'Ich',text:'Wir haben ihn großgezogen. Und dabei, ganz nebenbei, unser eigenes Zuhause gebaut.'}],()=>showTransition('ERINNERUNG 04 GESAMMELT','Kleine Pfoten. Große Erinnerungen.','Pablo wird größer. Die Jahreszeiten wechseln. Und irgendwo dazwischen wird aus gemeinsam verbrachter Zeit ein gemeinsames Leben.',()=>enterChapter(4),'In die Gegenwart →'))));}
  function openLaptop(){setPhase('laptop');objective('Öffne das Geschenk auf dem Laptop.');showDialog([{who:'Ich',text:'Jetzt schau einmal kurz vom Bildschirm weg. Ich sitze wirklich neben dir.'},{who:'Ich',text:'Dieses Spiel kann nicht alles zeigen, was wir erlebt haben. Es kann uns nur daran erinnern, wie viel da schon ist.'},{who:'Ich',text:'Und an das, was ich dir heute sagen möchte.'}],()=>setAction('Meine Nachricht öffnen',finishGame));}
  ...
  function drawClub(){
    ...
    if(s.phase==='dance'||s.phase==='dance-crash'){
      ...
      sprite(ctx,'me',s.player.x,s.player.y,226,{walk:!!s.target||keys.size>0,face:s.player.face});nameTag('Luana',lx,580,true);
  ...
  function drawClubIncident(){
    ...
    sprite(ctx,'luana',950,582,216);nameTag('Luana',950,592,true);
  ...
  function drawDrive(){
    ...
    ctx.beginPath();for(let y=-20;y<=H+20;y+=10){const cx=centreAt(y);y===-20?ctx.moveTo(cx-210,y):ctx.lineTo(cx-210,y);}
    ...
    ctx.restore();drawCountdown(d,'WEICHE DEN ANDEREN AUTOS AUS');
  }
  ...
  function drawHome(){if(s.phase==='laser'){drawPabloGame();return;}if(s.phase==='growing'&&s.phaseTime>=18){drawSofaMemory();return;}background('home');if(s.phase==='growing'){const t=s.phaseTime;const seasons=['Die ersten Tage.','Ein Sommer voller Unfug.','Draußen wird es Herbst.','Und plötzlich ist er groß.'];const si=Math.min(3,Math.floor(t/4.3));ctx.fillStyle=['#efd8970a','#a9bd7520','#b56d3d20','#bfd1ce20'][si];ctx.fillRect(0,0,W,H);sprite(ctx,'me',520,567,210);sprite(ctx,'luana',765,564,211);const growth=clamp(t/16,0,1);sprite(ctx,growth<.45?'kitten':'pablo',640,579,54+growth*38);text(ctx,seasons[si],640,180,36,'#f5ebcf','center','Georgia');round(ctx,474,211,332,4,2,'#faf4d44d');round(ctx,474,211,Math.max(2,332*growth),4,2,'#edcd95');for(let i=0;i<13;i++){const y=(i*60+t*24)%H,x=(i*133+Math.sin(t+i)*18)%W;ellipse(ctx,x,y,si===2?5:2,si===2?2:2,si===2?'#e6b26265':'#f9edbd40');}return;}const adult=s.chapter===4||s.phase==='home-grown';sprite(ctx,'luana',s.chapter===4?1060:767,s.chapter===4?477:557,206);if(s.chapter===4){sprite(ctx,'pablo',s.catX,s.catY,92,{walk:!!s.catTarget});}else{sprite(ctx,adult?'pablo':'kitten',s.catX,s.catY,adult?92:58,{walk:s.petTime>0});if(s.homeStep===3&&s.petTime>0){ellipse(ctx,s.catX-40-Math.sin(s.petTime*3)*55,581-Math.abs(Math.sin(s.petTime*4))*30,10,10,'#c7985f');}}sprite(ctx,'me',s.player.x,s.player.y,214,{walk:!!s.target||keys.size>0,face:s.player.face});if(s.chapter===3&&s.homeStep>=2){ellipse(ctx,362,588,37,12,'#e1d9bb');ellipse(ctx,362,585,30,8,'#9c7750');}if(!s.dialog.length){drawMarker(s.chapter===4?1140:s.catX,s.chapter===4?202:s.catY-15);}if(s.chapter===4){const glow=ctx.createRadialGradient(1142,242,4,1142,242,75);glow.addColorStop(0,'#d6edbf25');glow.addColorStop(1,'#d6edbf00');ctx.fillStyle=glow;ctx.fillRect(1067,167,150,150);}}
  function drawPabloGame(){
    background('home');const p=s.pabloGame;if(!p)return;
    sprite(ctx,'me',87,467,172);sprite(ctx,'luana',1207,467,174);
  ...
  canvas.addEventListener('pointerdown',e=>{if(s.paused||s.dialog.length||s.afterWalk||!$('transition').classList.contains('hidden'))return;const {x,y}=scenePoint(e);if(x<0||x>W||y<0||y>H)return;canvas.focus({preventScroll:true});if(s.phase==='laser'){moments.pointLaser(s.pabloGame,x,y);return;}if(s.phase==='dance'){if(y>=150&&y<=282&&x>=453&&x<=826)danceInput(clamp(Math.round((x-500)/93),0,3));return;}if(s.phase==='drive'){s.drive.targetX=x;return;}if(s.phase==='chase'){jump();return;}if(['club','home','home-grown','present','laptop'].includes(s.phase)){walkTo(clamp(x,75,1200),clamp(y,525,625),null);}});
  canvas.addEventListener('pointermove',e=>{if(s.phase!=='laser'||s.paused||s.dialog.length)return;if(e.pointerType==='touch'&&!e.buttons)return;const {x,y}=scenePoint(e);moments.pointLaser(s.pabloGame,x,y);});
