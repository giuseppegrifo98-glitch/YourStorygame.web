...
(() => {
  ...
  // Standing adults are about twice the sofa-back height. Keep the same
  // floor-depth perspective throughout the home, laser and final scenes.
  function homePerson(key,x,y,options={}){
    const depth=.88+.12*clamp((y-480)/110,0,1);
    sprite(ctx,key,x,y,(key==='me'?450:430)*depth,options);
  }
  function drawHome(){if(s.phase==='laser'){drawPabloGame();return;}if(s.phase==='growing'&&s.phaseTime>=18){drawSofaMemory();return;}background('home');if(s.phase==='growing'){const t=s.phaseTime;const seasons=['Die ersten Tage.','Ein Sommer voller Unfug.','Draußen wird es Herbst.','Und plötzlich ist er groß.'];const si=Math.min(3,Math.floor(t/4.3));ctx.fillStyle=['#efd8970a','#a9bd7520','#b56d3d20','#bfd1ce20'][si];ctx.fillRect(0,0,W,H);homePerson('me',500,590);homePerson('luana',800,587);const growth=clamp(t/16,0,1);sprite(ctx,growth<.45?'kitten':'pablo',640,579,54+growth*38);text(ctx,seasons[si],640,84,36,'#f5ebcf','center','Georgia');round(ctx,474,111,332,4,2,'#faf4d44d');round(ctx,474,111,Math.max(2,332*growth),4,2,'#edcd95');for(let i=0;i<13;i++){const y=(i*60+t*24)%H,x=(i*133+Math.sin(t+i)*18)%W;ellipse(ctx,x,y,si===2?5:2,si===2?2:2,si===2?'#e6b26265':'#f9edbd40');}return;}const adult=s.chapter===4||s.phase==='home-grown';homePerson('luana',s.chapter===4?760:800,s.chapter===4?510:580);if(s.chapter===4){sprite(ctx,'pablo',s.catX,s.catY,92,{walk:!!s.catTarget});}else{sprite(ctx,adult?'pablo':'kitten',s.catX,s.catY,adult?92:58,{walk:s.petTime>0});if(s.homeStep===3&&s.petTime>0){ellipse(ctx,s.catX-40-Math.sin(s.petTime*3)*55,581-Math.abs(Math.sin(s.petTime*4))*30,10,10,'#c7985f');}}homePerson('me',s.player.x,s.player.y,{walk:!!s.target||keys.size>0,face:s.player.face});if(s.chapter===3&&s.homeStep>=2){ellipse(ctx,362,588,37,12,'#e1d9bb');ellipse(ctx,362,585,30,8,'#9c7750');}if(!s.dialog.length){drawMarker(s.chapter===4?1140:s.catX,s.chapter===4?202:s.catY-15);}if(s.chapter===4){const glow=ctx.createRadialGradient(1142,242,4,1142,242,75);glow.addColorStop(0,'#d6edbf25');glow.addColorStop(1,'#d6edbf00');ctx.fillStyle=glow;ctx.fillRect(1067,167,150,150);}}
  function drawPabloGame(){
    background('home');const p=s.pabloGame;if(!p)return;
    homePerson('me',130,485);homePerson('luana',1150,485);
