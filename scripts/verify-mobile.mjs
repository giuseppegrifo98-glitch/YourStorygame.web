...
function element(id){if(elements.has(id))return elements.get(id);const classes=new Set(),events={};const e={style:{},dataset:{},events,textContent:'',classList:{add:x=>classes.add(x),remove:x=>classes.delete(x),contains:x=>classes.has(x),toggle(x,on){if(on)classes.add(x);else classes.delete(x);}},focus(){},setAttribute(){},setPointerCapture(){},getBoundingClientRect:()=>({left:0,top:0,width:138,height:138}),addEventListener(n,fn){(events[n]??=[]).push(fn);},fire(n,id=1){for(const fn of events[n]||[])fn({pointerId:id,preventDefault(){},clientX:130,clientY:69});}};elements.set(id,e);return e;}
...
const arcade=sandbox.window.MemoryArcade,state={screen:'play',phase:'drive',drive:arcade.createDrive(),paused:false},keys=new Set();let jumps=0;
const mobile=sandbox.window.MemoryMobile.create({state:()=>state,keys,blocked:()=>state.paused,audio:{on:false},pause(){},interact(){},jump(){jumps++;},danceInput(){}});
...
check('Start gates driving and hides instructions',()=>{assert(mobile.isBlocked());element('drive-start').onclick();assert(!mobile.isBlocked());assert(element('drive-start').classList.contains('hidden'));assert(!element('drive-controls').classList.contains('instructing'));});
...
check('Chase has one jump button and no joystick',()=>{state.phase='chase';mobile.sync();assert(element('joystick').classList.contains('hidden'));assert(!element('mobile-action').classList.contains('hidden'));});
check('Laser hint disappears on first touch',()=>{state.phase='laser';mobile.sync();assert(element('mobile-pad').classList.contains('hidden'));assert(!element('laser-hint').classList.contains('hidden'));element('game').fire('pointerdown');assert(element('laser-hint').classList.contains('hidden'));});
check('Jump button supports keyboard activation',()=>{state.phase='chase';mobile.sync();element('mobile-action').events.click[0]({detail:0});assert.equal(jumps,1);});
check('Returning to driving shows a fresh start instruction',()=>{state.phase='drive';mobile.sync();assert(mobile.isBlocked());assert(!element('drive-start').classList.contains('hidden'));});
